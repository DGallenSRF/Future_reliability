...
model_max <- model_max %>% 
  ...
  #        quantiles == '90%' | quantiles == '95%') %>%
  mutate(Cong_Density = replace(Cong_Density, Cong_Density == 76, 40)) %>%
  mutate(Cong_Density = replace(Cong_Density, is_Delay == 'No_Delay' & quantiles == '75%', 32)) %>%
  mutate(Cong_Flow = replace(Cong_Flow, is_Delay == 'No_Delay' & quantiles == '75%', 1136)) %>%
  as.data.frame()
...
formulaExp <- as.formula(TTI ~(DC <= .55) + (DC > .55) * ( mu * (DC ^ b)))
b <- 3
mu <- 1.5
...
# 1S
# breaks <- model_max %>% data.frame() %>%
#   mutate(br = c(0.95,0.96,0.96,0.98,1.02,1.01,0.99,
#                 0.89,0.89,0.89,0.9,0.92,0.94,0.94)) %>%
#   select(is_Delay,quantiles,br)

# 2S
breaks <- model_max %>% data.frame() %>%
  mutate(br = c(0.84,0.81,0.81,.76,0.7,0.66,0.62,
                0.9,0.85,0.89,0.72,0.55,0.57,0.48)) %>%
  select(is_Delay,quantiles,br)
...
write.csv(Rel_2014_p,'4_N_predict.csv',row.names = FALSE)
getwd()
```

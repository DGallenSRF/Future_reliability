...
#   arrange(Segment)
dir()
Segments_494 <- read.csv('Segments.csv')

Rel <- Data_494 %>% 
  mutate(Hour        = as.factor(format(DateTime,format = '%H')),
         YearDay     = yday(DateTime)) %>%
  group_by(Segment, YearDay, Hour) %>%
  summarise(VMT_hr  = mean(VMT_total),
            TT_hr   = mean(TT_mean),
            weather = any(weather == 1),
            Crash   = any(Crash == 1)) %>%
  merge(Segments_494, by = 'Segment') %>%
  mutate(Vol_15_min  = VMT_hr/Length,
         Speed       = Length/TT_hr*60,
...
Rel <- Rel %>% 
  mutate(Delay_type  = (ifelse(weather == TRUE, 
                               'Weather',
                               ifelse(Crash == 1, 
                                      'Crash',
                                      'None'))),
...
ggplot(Rel,aes(x = as.factor(Segment),y = TT_hr)) +
...
Rel_bin <- Rel %>%
  ...
  summarise(quantiles = list(sprintf("%1.0f%%", p*100)),
            TT_quantile = list(quantile(TT_hr,p))) %>%
  unnest() %>%
  merge(.,Segments_494, by = 'Segment') %>%
...
Rel_bin <- Rel_bin %>%
  ...
  filter(!(Segment == 1 & quantiles == '5%' & Delay_type == 'Crash' & Density_bin == 70)) %>%
  filter(!(Segment == 1 & quantiles == '5%' & Delay_type == 'Crash' & Density_bin == 36)) %>%
  filter(!(Segment == 1 & quantiles == '10%' & Delay_type == 'Crash' & Density_bin == 36)) %>%
   # filter(!(Segment == 1 & quantiles == '95%' & Delay_type == 'Crash' & Density_bin >59)) %>%
   #  filter(!(Segment == 1 & quantiles == '95%' & Delay_type == 'Crash' & Density_bin == 36)) %>%
   #  filter(!(Segment == 1 & quantiles == '95%' & Delay_type == 'Crash' & Density_bin == 46)) %>%
  filter(!(Segment == 1 & quantiles == '5%' & Delay_type == 'None' & Density_bin == 80)) %>%
  filter(!(Segment == 1 & quantiles == '5%' & Delay_type == 'None' & Density_bin == 100)) %>%
  filter(!(Segment == 1 & quantiles == '5%' & Delay_type == 'Weather' & Density_bin == 70)) %>%
  filter(!(Segment == 1 & quantiles == '5%' & Delay_type == 'Weather' & Density_bin == 80))
...
dens_flow_plot(7,'none', bin = TRUE)
```
...
dens_flow_plot(8,'none', bin = TRUE)
```
...
Rel_bin <- Rel_bin %>%
  filter(!(Segment == 9 & Delay_type == 'Weather' & Density_bin > 59)) %>%
...
```{r calculate Demand, include=FALSE}
...
sample_data <- Rel_bin %>%
  filter(Segment == 8, Delay_type == 'None', quantiles == '50%', !is.na(DC)) %>%
  select(DC, TTI)
...
Rel_q <- Rel_p %>% 
  select(Segment, Delay_type, Density_bin, TT_q = TT_hr, Index) %>%
  data.table()
...
Rel_p <- merge(Rel_p, 
                 join_TT[,c('Index','TT_quantile','Quantiles','Flow_cong','Density_cong','FF','br')],
                 by = 'Index', all.x=TRUE)

table(is.na(Rel_p$Flow_cong))

Rel_p <- Rel_p %>%
  mutate(Demand  = ifelse(Density_bin <= Density_cong,Hourly_Flow,2*Flow_cong - Hourly_Flow),
         DC = Demand/Flow_cong,
         TTI = TT_hr/(Length/FF * 60)) %>%
  nest(-Delay_type, -Quantiles)
  

Rel_p <- merge(Rel_p,model_nls %>% select(Delay_type, Quantiles = quantiles, fit)) %>%
  as.tibble() %>%
  mutate(pred = map2(fit,data,predict)) %>%
  as.tibble() %>%
  unnest(data,pred)

Rel_p$Diff <- round(abs(Rel_p$TTI - Rel_p$pred),2)

diff_data <- filter(Rel_p, Diff > 1) %>% select(-pred, everything())

rm(join_TT, Rel_q, TT_q)
```
...
plot_predict <- function(Seg){
  pr <- ggplot(filter(Rel_p,Segment == Seg), aes(x=TTI,y=pred, color=Quantiles))+
  geom_point()+
...
TT_heat <-function(data,seg, model){ 
  ...
  heat <- data %>%
    filter(Segment == seg) %>% 
    select(TT_hr, Length, pred, YearDay, Hour) %>% 
    mutate(mod = model,
           TTI = ifelse(mod == TRUE,
                        pred,
                        TT_hr/(Length/60 * 60)),
...
g <- ggplot(heat) +
  geom_tile(aes(x=YearDay,y=Hour,color = TTI_Cat, fill = TTI_Cat))  +

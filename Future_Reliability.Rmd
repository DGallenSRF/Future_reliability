...
Rel_p <- merge(Rel_p, select(Rel_p_u, Index, pred_u = pred), all.x = TRUE) %>%
  ...
  mutate(TT_quantile_l = ifelse(is.na(TT_quantile_l),
         ...
         TTI = TT_mean / (Length / 60)
         pred_final = ifelse(is.na(pred_l), pred_u, pred_l * (1+Diff)))
...
plot_predict <- function(Seg){
  ...
  pr <- ggplot(filter(Rel_p,Segment == Seg), aes(x= TTI,y=pred_final))+
    geom_point() +
...
TT_heat <-function(data,seg, model){ 
  ...
  heat <- data %>%
    filter(Segment == seg) %>% 
    select(DateTime, TTI, Length, pred = pred_final) %>% 

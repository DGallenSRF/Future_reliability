...
Rel_2014 <- dat %>% mutate(Vol_15_min  = VMT_total/Dist1,
                           ...
                           Delay_type  = (ifelse(Weather == 1, 
                                                ...
                                                ifelse(Crash == 1, 
                                                       'Crash',
                                                       'None'))),
                           Delay       = ifelse(Delay_type == 'None', 'No_Delay', 'Delay'),
...
Rel_2014 %>%
  group_by(Density_bin,Delay) %>%
  summarise(Count = n()) %>%
  arrange(Density_bin) %>%
  dcast(Density_bin ~ Delay)
```
...
model_max <- model %>%
  ...
  select(Delay, quantiles, Cong_Flow = .fitted, Cong_Density = Density_bin)
...
f <- ggplot()+
  ...
  geom_smooth(data=dat,
              aes(x= Density_bin,y = Flow, color= quantiles),span =0.75,se=F)+
  geom_vline(data=filter(model_max,Delay == 'No_Delay'),aes(xintercept=Cong_Density, color = quantiles)) +
...
```{r calculate demand}
Rel_2014_bin_dt <- merge(Rel_2014_bin_dt,model_max,by = c('Delay','quantiles')) %>%
...
s <- ggplot()+
  ...
  geom_smooth(data=Rel_2014_bin_dt,aes(x= Demand,y = TTI, color= quantiles),se=F,span = 0.75)+
...
sample_data <- Rel_2014_bin_dt %>%
  filter(Delay == 'No_Delay',
         quantiles == '95%') %>%
  select(DC, TTI)


formulaExp <- as.formula(TTI ~(DC <= .2) + (DC > .2) * ( mu * (DC ^ b)))
b <- 3
mu <- 1.5
...
# 3N
breaks <- model_max %>% data.frame() %>%
  mutate(br = c(0.95,0.93,0.96,0.44,0.34,0.37,0.35,
                0.88,0.79,0.88,0.46,0.3,0.2,0.3)) %>%
  select(Delay,quantiles,br)

# 4N
# breaks <- model_max %>% data.frame() %>%
#   mutate(br = c(0.77,0.74,0.75,0.7,0.64,0.58,0.2,
#                 0.8,0.67,0.82,0.3,0.17,0.2,0.1)) %>%
#   select(Delay,quantiles,br)
...
Rel_2014_TT_dt <- Rel_2014 %>%
  select(Delay,Density_bin,TT_quantile = TT_mean, Index) %>%
...
setkey(Rel_2014_bin_dt,Delay,TT_quantile,Density_bin)
...
Rel_2014_f <- merge(Rel_2014_f,model_nls %>% select(Delay, Quantiles = quantiles, fit)) %>%
  as.tibble() %>%

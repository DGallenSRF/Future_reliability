...
sample_data <- Rel_p %>%
  filter(Segment == S, Delay_type == D_type, Quantile_l == q, !is.na(DC)) %>%
  select(DC, TTI) 

knot_list <- c(0.55, 0.9)
# sample_data$TTI <- ifelse(sample_data$DC < knot_list[1], 1, sample_data$TTI)
# sample_data <- rbind(data.frame(DC = seq(0.1,knot_list[1],0.005), TTI = 1), sample_data)
formulaExp <- lm(TTI ~ splines::bs(DC, knots = knot_list), data = sample_data)

# sample_data$fitted <- ifelse(sample_data$DC < knot_list[1], 1, formulaExp$fitted.values)
sample_data$fitted <- formulaExp$fitted.values

ggplot(sample_data) +
  geom_point(aes(x = DC, y = TTI), alpha = 0.25) +
  geom_line(aes(x = DC, y = fitted, color = 'red')) +
  scale_x_continuous(breaks = seq(0.1,4,0.1)) +
  scale_y_continuous(breaks = seq(1,max(sample_data$TTI),0.25))+

...
throughput = alternative %>%
  ...
  filter(DayPeriod %in% dp,
         weekday %in% wd) %>%
  select(DateTime, Segment, weekday, VMT_2040_15, doy, TTI = pred_final, direction, Length, FF,DayPeriod) %>%
  mutate(TT_hour = (TTI * Length) / FF) %>%
  merge(occ_dat)
...
occ <- occ %>%
  mutate(weekperiod = 'weekend',
         occupancy = 1.07) %>%
  bind_rows(occ)
# occ[grepl('*ff',occ$Alternative), 'occupancy'] <- 1.84
```
...
write.csv(pmt_pht_494, 'Final tables/pmt_pht/pmt_pht_494_2040_allday_7_23.csv')

...
dat <- dens_flow_tab[dens_flow_tab$Segment == 1,]

ggplot()+
  geom_point(data=Rel_bin, aes(x = Density_bin, y = Flow, color = quantiles)) +
  geom_segment(data = dat, aes(x = Density_cong, y = Axis, xend = Density_cong, yend = Flow_cong, color = quantiles)) +
    geom_segment(data = dat, aes(x = Axis, y = Flow_cong, xend = Density_cong, yend = Flow_cong, color = quantiles) ) +
  # geom_smooth(span=0.75,se=F)+
  facet_grid(~Delay_type, scales = 'free')+
  # scale_y_continuous(minor_breaks = seq(0, 4000,50), breaks = seq(0,4000,100))+
  # scale_x_continuous(minor_breaks = seq(0,500,5), breaks = seq(0,500,10))+
  # theme(legend.position=legend)+
  ggtitle(paste('Segment:',1))
...
dens_flow_plot(3,'none', bin = TRUE)
```
...
sample_data <- Rel_bin %>%
  filter(Segment == 9, Delay_type == 'Weather', quantiles == q, !is.na(DC)) %>%
  select(DC, TTI)
...
TT_q_table <- data.table(Rel_bin) %>% select(Segment, Delay_type, Density_bin, TT_q = TT_quantile, quantiles, FF_quantile, Flow_cong, Density_cong)
...
Rel_p <- Rel_p %>%
  mutate(Demand_u  = ifelse(Density_bin <= Density_cong_u, Hourly_Flow, 2 * Flow_cong_u - Hourly_Flow),
         DC_u = Demand_u / Flow_cong_u,
         TTI_u = TT_mean / (Length/FF_u * 60),
         Demand_l = ifelse(Density_bin <= Density_cong_l, Hourly_Flow, 2 * Flow_cong_l - Hourly_Flow),
         DC_l = Demand_l / Flow_cong_l,
         TTI_l = TT_mean / (Length/FF_l * 60))  

Rel_p_u <- Rel_p %>% 
  select(Segment, Delay_type, Quantiles = Quantile_u, DC = DC_u, Index) %>%
  nest(-Segment, -Delay_type, -Quantiles) %>%
...
Rel_p_l <- Rel_p %>% 
  select(Segment, Delay_type, Quantiles = Quantile_l, DC = DC_l, Index) %>%
  nest(-Segment, -Delay_type, -Quantiles) %>%
  merge(model_nls %>% select(Segment, Delay_type, Quantiles = quantiles, fit)) %>%
  as.tibble() %>%
  mutate(pred = map2(fit,data,predict)) %>%
...
Rel_p <- merge(Rel_p, select(Rel_p_u, Index, pred_u = pred), all.x = TRUE) %>%
  ...
  mutate(TT_quantile_l = ifelse(is.na(TT_quantile_l),
         ...
         # TTI = ifelse(is.na(FF_l),
         #              TT_mean / (Length / FF_u * 60),
         #              TT_mean / (Length / (FF_l * (1+Diff)) * 60)),
         TTI = TT_mean/(Length/60 * 60),
         pred_final = ifelse(is.na(pred_l), 
                             pred_u,
...
Rel_p %>% 
  filter(Segment == 3) %>%
  mutate(Diff = round(abs(TTI - pred),2)) %>%
...
```{r}
x <- Rel_p %>% filter(Segment == 1) %>% select(pred_final, TTI) 
as.data.frame(table(cut(x$pred_final, breaks=seq(0,5, by=0.1))))
write.csv(x,'dat.csv', row.names = FALSE)
getwd()
```

...
sample_data <- Rel_bin %>%
  filter(Segment == 9, Delay_type == 'None', quantiles == '95%', !is.na(DC)) %>%
  select(DC, TTI)
...
TT_q_table <- data.table(Rel_bin) %>% select(-quantiles,TT_q = TT_quantile, quantiles)
...
Rel_p_u <- Rel_p %>% 
  select(Delay_type, Quantiles = Quantile_u, DC = DC_u, Index) %>%
  nest(-Delay_type, -Quantiles) %>%
...
Rel_p_l <- Rel_p %>% 
  select(Delay_type, Quantiles = Quantile_l, DC = DC_l, Index) %>%
  nest(-Delay_type, -Quantiles) %>%
  merge(model_nls %>% select(Delay_type, Quantiles = quantiles, fit)) %>%
  as.tibble() %>%
  mutate(pred = map2(fit,data,predict)) %>%
...
Rel_p <- merge(Rel_p, select(Rel_p_u, Index, pred_u = pred)) %>%
  merge(select(Rel_p_l, Index, pred_l = pred))
...
TT_heat <-function(data,seg, model){ 
  ...
  heat <- data %>%
    filter(Segment == seg) %>% 
    select(DateTime, TT_mean, Length, pred) %>% 

...
TT_q_table <- data.table(Rel_bin) %>% 
  select(Segment, Delay_type, Density_bin,
         ...
         FF_quantile, Flow_cong, Density_cong) %>% data.table()
...
Rel_p <- merge(Rel_p, 
                 join_TT[,c('Index','TT_quantile_u', 'TT_quantile_l',
                            ...
                            'Flow_cong_l','Density_cong_l','FF_l')],
                 by = 'Index', all.x=TRUE)
...
# 
# sample_data <- Rel_p %>%
#   filter(Segment == 7, Delay_type == 'None', Quantile_l == q, !is.na(DC)) %>%
#   select(DC, TTI, TT_mean)
# 
# # sample_data <- Rel_bin %>%
# #   filter(Segment == 7, Delay_type == 'None', quantiles == q, !is.na(DC)) %>%
# #   select(DC, TTI)
# 
# formulaExp <- as.formula(TTI ~(DC <= br1) +( DC > br1) * ( mu * (DC ^ b)))
# b <- 3
# mu <- 1.5
# # nu <-  .1
# 
...
sample_data <- Rel_p %>%
  filter(Segment == S, Delay_type == D_type, Quantile_l == q, !is.na(DC), Index) %>%
  select(DateTime, TT_mean, VMT_total, Segment, Delay_type, Quantile_l, DC, TTI, Index) 

model_data <- sample_data
knot_list <- c(0.6)
model_data$TTI <- ifelse(sample_data$DC < knot_list[1], 1, sample_data$TTI)
# model_data <- bind_rows(data.frame(DC = seq(0.1,knot_list[1],0.005), TTI = 1), model_data)
formulaExp <- lm(TTI ~ splines::bs(DC, knots = knot_list), data = model_data)

# sample_data$fitted <- ifelse(sample_data$DC < knot_list[1], 1, formulaExp$fitted.values)
model_data$fit <- formulaExp$fitted.values
sample_data <- merge(sample_data, select(model_data, Index, fit), by = 'Index', all.x = TRUE) %>%
  nest(-Segment,-Delay_type, -Quantile_l)

ggplot(model_data) +
  geom_point(aes(x = DC, y = TTI), alpha = 0.25) +
  geom_line(aes(x = DC, y = fit, color = 'red')) +
  scale_x_continuous(breaks = seq(0.1,4,0.1)) +
  scale_y_continuous(breaks = seq(1,14,0.1)) +
  # geom_abline(slope = 0, intercept = 1) +
  geom_vline(xintercept = knot_list)

iter_breaks <- data.frame(S=S,D_type=D_type,q=q)
iter_breaks$knot <- list(knot_list)
iter_breaks$fit <- list(formulaExp)


bind_model_breaks <- function(x) {
  x <- x
  ifelse(nrow(filter(model_breaks,
                    model_breaks$S %in% x$S,
                    model_breaks$D_type %in% x$D_type,
                    model_breaks$q %in% x$q)) > 0,
         stop('Data exists'),
         return(x))
}

r <- bind_model_breaks(iter_breaks)
summary(formulaExp)

```

```{r}
# pred_data <- sample_data

model_breaks <- rbind(model_breaks,r)
pred_data <- rbind(pred_data, sample_data)

# model_breaks <- model_breaks %>% droplevels() %>%
#   mutate(knot = replace(knot, S == iter_breaks$S &
#                           D_type == iter_breaks$D_type &
#                           q == iter_breaks$q,
#                         list(iter_breaks$knot[[1]])),
#          fit = replace(fit, S == iter_breaks$S &
#                          D_type == iter_breaks$D_type &
#                          q == iter_breaks$q,
#                        list(iter_breaks$fit[[1]])))

saveRDS(model_breaks, 'model_breaks.rds')

```

```{r}
func_exp <- function(x,y) {
lm(TTI ~ splines::bs(DC, knots = x), data = y)
}


mod1 <- Rel_p %>%
  filter(Segment == 1) %>%
  select(Segment, Delay_type, Quantiles = Quantile_u, DC, Index, TTI) %>%
  nest(-Segment, -Delay_type, -Quantiles) %>%
  merge(model_breaks %>% select(Segment = S, Delay_type = D_type, Quantiles = q, knot)) %>%
  mutate(fit = purrr::map2(knot,data,func_exp))
```


```{r create old prediction model, include = FALSE}

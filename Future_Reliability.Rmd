...
sample_data <- Rel_p %>%
  filter(Segment == 3, Delay_type == 'Weather', Quantile_l == q, !is.na(DC)) %>%
  select(DC, TTI)
...
Rel_p_u <- Rel_p %>% 
  select(Segment, Delay_type, Quantiles = Quantile_u, DC, Index) %>%
...
Rel_p_l <- Rel_p %>% 
  select(Segment, Delay_type, Quantiles = Quantile_l, DC, Index) %>%
  nest(-Segment, -Delay_type, -Quantiles) %>%
  merge(model_nls %>% select(Segment, Delay_type, Quantiles = quantiles, fit)) %>%
  as.tibble() %>%
  mutate(pred = map2(fit,data,predict)) %>%

...
g <- ggplot(heat) +
  ...
  colScale + fillScale +
  ggtitle(paste('Segment', seg, 'TTI Heatmap','-',predicted_TF,alternative))
return(g)
}
```
...
DayPeriods <- data.frame(Hour = c('5','6','7','8','9'), DayPeriod = 'Morning') %>%
  bind_rows(data.frame(Hour = c('10','11','12','13'), DayPeriod = 'Lunch')) %>%
  bind_rows(data.frame(Hour = c('14','15','16','17','18'), DayPeriod = 'Afternoon')) %>%
  bind_rows(data.frame(Hour = c('19','20','21','22','23'), DayPeriod = 'Evening')) %>%
  bind_rows(data.frame(Hour = c('0','1','2','3','4'), DayPeriod = 'Night'))

Rel_p <- merge(Rel_p, DayPeriods)
```
...
my_cols <- c('Segment', 'DateTime', 'Delay_type', 'Quantile_u',
             ...
             'Index', 'br','factor', 'Lanes', 'Diff', 'TTI', 'TT_mean','Length')

DayPeriods <- data.frame(Hour = c(6,7,8,9), DayPeriod = 'Morning') %>%
  bind_rows(data.frame(Hour = c(11,12,13,14), DayPeriod = 'Early_afternoon')) %>%
  bind_rows(data.frame(Hour = c(15,16,17,18), DayPeriod = 'Afternoon')) %>%
  bind_rows(data.frame(Hour = c(19,20,21,22,23), DayPeriod = 'Evening')) %>%
  bind_rows(data.frame(Hour = c(0,1,2,3,4,5), DayPeriod = 'Night'))

Exist_Demand <- Rel_p %>%
  select(one_of(my_cols))%>%
  mutate(Hour = hour(DateTime),
         Demand_Hour_all_lanes = Demand_Hour_Lane * Lanes) %>%
  merge(DayPeriods)
```
...
MNPass_2040 <- forecasts %>%
  ...
  select(Segment, Demand_2040_change, Lanes_add) %>%
  merge(Exist_Demand) %>%
  mutate(Time_Period = ifelse(DayPeriod %in% c('Morning','Afternoon'), DayPeriod, 'Off_Peak')) %>%
  merge(MNPass_eqt) %>%
  mutate(Demand_Hour_2040_all_lanes  = Demand_Hour_all_lanes * Demand_2040_change,
...
vehicle_tru <- function(alternative){
...
d <- Rel_p %>%
  ...
  mutate(Flow_Hour_2040_all_lanes = Hourly_Flow_per_lane * Demand_2040_change * (Lanes+Lanes_add)) %>%
...
vehicle_tru_mnPass_ff <- function(alternative){
...
vehicle_tru_mnPass_remain <- function(alternative){
...
d <- Rel_p %>%
  ...
  mutate(Flow_Hour_2040_remain_lanes = Hourly_Flow_per_lane * Demand_2040_change * (Lanes)) %>%
  group_by(Segment,DayPeriod, doy) %>%
  summarise(Demand_Hour_2040_all_lanes_day_dp_sum = sum(Remaining_all_lanes_demand_hour),
            Flow_Hour_2040_remain_lanes_day_dp_sum = sum(Flow_Hour_2040_remain_lanes),
            Length = mean(Length), FF = round(mean(FF),0)) %>%
  mutate(demand_flow_factor = Demand_Hour_2040_all_lanes_day_dp_sum /
...
throughput <- alternative %>%
  ...
  mutate(TT = (TTI * Length) / FF) %>%
  group_by(DateTime, Segment) %>%
  summarise(VMT_direction_15min = sum(VMT_2040_15),
            TT_direction_15min = sum(TT),
            direction_length  = sum(Length),
            FF_direction_15min = mean(FF)) %>%
  mutate(FF_TT_direction = direction_length / FF_direction_15min,
         TTI = TT_direction_15min / FF_TT_direction) %>%
...
```{r}
GP_2040_spare <- GP_2040
# GP_2040 <- GP_2040_spare
```

```{r}
occ <- readxl::read_xlsx("H:/Projects/11000/11155/TraffStudy/Reliability/Model Occupancy.xlsx")
occ <- occ[2:dim(occ)[1],1:2]
colnames(occ) <- c('Alternative','occupancy')
...
thru_no_change <-  vehicle_tru(no_change_2040)
thru_no_change <- agg_vehicle_thru(thru_no_change, no_change_2040,agg)
thru_no_change$occupancy <- thru_no_change$Vol * as.numeric(occ[occ$Alternative == 'No-Build (2040)','occupancy'])
thru_no_change$Alternative <- 'no_change'

GP_2040$Demand_2040_change <- GP_2040$Demand_2040_change
thru_GP <-  vehicle_tru(GP_2040)
thru_GP <-  agg_vehicle_thru(thru_GP, GP_2040, agg)
thru_GP$occupancy <- thru_GP$Vol * as.numeric(occ[occ$Alternative == 'General Purpose Lane (2040)','occupancy'])
thru_GP$Alternative <- 'GP'

thru_MNPass_ff <- vehicle_tru_mnPass_ff(MNPass_2040)
thru_MNPass_ff <- select(MNPass_2040_FF, -Demand_Hour_2040_all_lanes) %>%
  ...
  agg_vehicle_thru(alternative = .,thru_MNPass_ff, agg)
thru_MNPass_ff$occupancy <- thru_MNPass_ff$Vol * 
  as.numeric(occ[occ$Alternative == 'MnPass (2040) – on MnPass lane','occupancy'])
thru_MNPass_ff$Alternative <- 'MNPass'  
  
thru_MNPass_remain <- vehicle_tru_mnPass_remain(MNPass_2040)
thru_MNPass_remain <- select(MNPass_2040_remaining_lanes, -Demand_Hour_2040_all_lanes) %>%
  ...
  agg_vehicle_thru(alternative = .,thru_MNPass_remain, agg)  
thru_MNPass_remain$occupancy <- thru_MNPass_remain$Vol * 
  as.numeric(occ[occ$Alternative == 'MnPass (2040) – on general purpose lanes','occupancy'])
thru_MNPass_remain$Alternative <- 'MNPass'

final_thru <- rbind(thru_GP, thru_no_change,thru_MNPass_remain, thru_MNPass_ff)

# thru_MNPass_ff,thru_MNPass_remain, 
...
ggplot(final_thru, aes(x=Alternative, y=occupancy, fill=TTI_Cat)) +
  ...
  colScale + fillScale +ggtitle('Reliability by Vehicles - AM PM Peak Periods - Average Weekday')
```

...
names(myColors) <- levels(TT_heat$TTI_Cat)
...
therm <- TT_heat %>%
  summarise(quantiles = list(sprintf("%1.1f%%", quants*100)),
            TT_quantile = list(quantile(TT_mean,quants))) %>%
  unnest() %>%
  select(quantiles, TTI = TT_quantile)

therm_table <- flextable(therm)
# theme_vanilla(therm_table)

bg(therm_table, i = ~ TTI  > 1.5, j =  ~ TTI, bg = myColors[[9]])
      # ~ TTI <= 4.0 & TTI > , color = myColors[[2]],
      # ~ TTI <= 3.5 & TTI > 3.0, color = myColors[[3]],
      # ~ TTI <= 3.0 & TTI > 2.5, color = myColors[[4]],
      # ~ TTI <= 2.5 & TTI > 2.0, color = myColors[[5]],
      # ~ TTI <= 2.0 & TTI > 1.5, color = myColors[[6]],
      # ~ TTI <= 1.5 & TTI > 1.25, color = myColors[[7]],
      # ~ TTI <= 1.25 & TTI > 1.0, color = myColors[[8]],
      # ~ TTI <= 1.0 , color = myColors[[9]])

...
colScale <- scale_colour_manual(name = "TTI_Cat",values = myColors, drop=FALSE)
fillScale <- scale_fill_manual(name = "TTI_Cat",values = myColors, drop=FALSE)
break_y_axis <- c('03:00','06:00','09:00','12:00','15:00','18:00','21:00')
...
g <-   ggplot(heat) +
  ...
  scale_y_discrete(breaks = break_y_axis) +
  xlab('Month') + ylab('Time of Day')+
  ...
  ggtitle(paste('Segment', seg, 'TTI Heatmap','-',alternative))
return(g)
}


TT_dir_heat <-function(data,seg, model, alternative = ' '){ 
...
Rel_p_dir$Hour <- hour(Rel_p_dir$DateTime)
Rel_p_dir$Hour <- as.numeric(Rel_p_dir$Hour)
Rel_p_dir$minute <- minute(Rel_p_dir$DateTime)
Rel_p_dir$hr_min <- Rel_p_dir$Hour + Rel_p_dir$minute/60


Rel_p_dir$DayPeriod <- ifelse((Rel_p_dir$hr_min >= 5.0) & (Rel_p_dir$hr_min < 10.0), 'Morning',
                          ifelse((Rel_p_dir$hr_min >=10.0) & (Rel_p_dir$hr_min < 14.0), 'Lunch',
                                 ifelse((Rel_p_dir$hr_min >= 14.0) & (Rel_p_dir$hr_min < 19.0),'Afternoon',
                                        ifelse((Rel_p_dir$hr_min >= 19.0) & (Rel_p_dir$hr_min < 23), 'Evening',
                                               ifelse((Rel_p_dir$hr_min >= 23) | (Rel_p_dir$hr_min < 5.0), 'Night','NA')))))


TT_therm <- Rel_p_dir %>% ungroup() %>%
  dplyr::rename(Segment = direction) %>%
# TT_therm <- Rel_p %>%
  filter(Segment == segment,
         DayPeriod %in% c('Morning','Afternoon')) %>%
  mutate(YearDay = yday(DateTime)) %>%
  select(DateTime, TTI , YearDay, DayPeriod, Segment)
...
add_time_periods <- function(d) {
  
d$Hour <- hour(d$DateTime)
d$Hour <- as.numeric(d$Hour)
d$minute <- minute(d$DateTime)
d$hr_min <- d$Hour + d$minute/60


d$DayPeriod <- ifelse((d$hr_min >= 5.0) & (d$hr_min < 10.0), 'Morning',
                          ifelse((d$hr_min >=10.0) & (d$hr_min < 14.0), 'Lunch',
                                 ifelse((d$hr_min >= 14.0) & (d$hr_min < 19.0),'Afternoon',
                                        ifelse((d$hr_min >= 19.0) & (d$hr_min < 23), 'Evening',
                                               ifelse((d$hr_min >= 23) | (d$hr_min < 5.0), 'Night','NA')))))
  return(data.frame(d))
}
...
# 
TT_therm <- add_time_periods(agg_mnpass_remain) %>%
  dplyr::rename(pred_final = TTI) %>%
# TT_therm <- Rel_p_dir %>% ungroup() %>%
  dplyr::rename(Segment = direction) %>%
  filter(Segment == segment,
         DayPeriod %in% c('Morning','Afternoon')) %>%
...
# remaining GP lanes
MNPass_eqt <- read.csv('./mnpass_equations_vissim_calibrated.csv') %>%
...
# MNPass_eqt$Beta <- MNPass_eqt$Beta + .0002
```
...
no_change_2040 <- forecasts %>%
  ...
  mutate(Time_Period = ifelse((DayPeriod %in% c('Morning','Afternoon')) & (weekdays(DateTime) %in% wkday),
                              DayPeriod, 'Off_Peak'))  %>%
...
mnpass <- filter(MNPass_2040,
       (MNPass_demand_hour_final/(Remaining_per_lane_demand_hour)<10) & 
         (MNPass_demand_hour_final/(Remaining_per_lane_demand_hour) >0)) %>%
  mutate(y = MNPass_demand_hour_final/Remaining_per_lane_demand_hour) %>%
  arrange(desc(y))

filter(MNPass_2040,
       (MNPass_demand_hour_final/(Remaining_per_lane_demand_hour)<10) & 
         (MNPass_demand_hour_final/(Remaining_per_lane_demand_hour) >0)) %>%
ggplot(aes(x = MNPass_demand_hour_final, y = MNPass_demand_hour_final/Remaining_per_lane_demand_hour,
                        color = Segment))+
  geom_jitter(alpha=1)+
...
d <- Rel_p %>%
  ...
  filter(DayPeriod %in% dp) %>%
  select(Segment, VMT_total_SRF, Hourly_Flow_per_lane , WeekDay, DateTime, Hour, Lanes, DayPeriod, doy, Length, FF) %>%
  merge(select(alternative, Segment, DateTime, Demand_2040_change, 
               Lanes_add, Remaining_all_lanes_demand_hour,Demand_Hour_2040_all_lanes)) %>%
  mutate(Flow_Hour_2040_all_lanes = Hourly_Flow_per_lane * Demand_2040_change * Lanes) %>%
  group_by_('Segment',grpby, 'doy') %>%
  summarise(Demand_Hour_2040_all_lanes_day_dp_sum = sum(Demand_Hour_2040_all_lanes),
            Flow_Hour_2040_all_lanes_day_dp_sum = sum(Flow_Hour_2040_all_lanes),
...
agg_vehicle_thru <- function(input_d, alternative, group, 
                             dp = c('Morning','Afternoon'), wkday = wkday){
...
throughput <- alternative %>%
  ...
  select(DateTime, Segment, weekday, VMT_2040_15, doy, TTI = pred_final, direction, Length, FF) %>%
  filter(weekday %in% wkday) %>%
...
PMT_PHT <- function(input_d, alternative,dp = allday,
                    grp = 'direction',wd = allweek,occ_dat) {

#
# input_d <-  vehicle_no_change
# alternative <- no_change_2040
# occupancy <- occ_mnpass_gp
...
#   select(occupancy,DayPeriod,weekperiod)
...
throughput = alternative %>%
  ...
  mutate(Flow_2040 = Demand_Hour_2040_all_lanes / demand_flow_factor,
         ...
         weekday=weekdays(.$DateTime),
         direction = ifelse(Segment < 6,'EB','WB'),
         weekperiod = ifelse(weekday %in% wkday,'weekday','weekend')) %>%
  filter(DayPeriod %in% dp,
         weekday %in% wd) %>%
  select(DateTime, Segment, weekday, VMT_2040_15, doy, TTI = pred_final, direction, Length, FF,DayPeriod,weekperiod) %>%
  mutate(TT_hour = (TTI * Length) / FF) %>%
  merge(occ_dat, by = c('DayPeriod','weekperiod'))
...
```{r, fig.width=10}
segment = 9
TT_heat(Rel_p,segment, model = FALSE, 'Existing') 
ggsave(paste('S',segment,'Existing.png'))
TT_heat(Rel_p,segment, model = TRUE, 'Modeled Existing')
ggsave(paste('S',segment,'Modeled Existing.png'))
TT_heat(no_change_2040,segment, model = TRUE,'No Build 2040')
ggsave(paste('S',segment,'No Build 2040.png'))
TT_heat(GP_2040,segment, model = TRUE, 'GP 2040')
ggsave(paste('S',segment,'GP 2040.png'))
TT_heat(MNPass_2040_FF,segment, model = TRUE, 'MNPass 2040')
ggsave(paste('S',segment,'MNPass 2040.png'))
TT_heat(MNPass_2040_remaining_lanes,segment, model = TRUE, 'MNPass 2040 GP lanes')
ggsave(paste('S',segment,'MNPass 2040 GP lanes.png'))
```
...
occ$weekperiod <- 'weekday'
occ <- occ %>%
  mutate(weekperiod = 'weekend',
         occupancy = 1.07) %>%
  bind_rows(occ)
occ[grepl('*ff',occ$Alternative), 'occupancy'] <- 1.64
```


```{r no change}

occ_no_change <- as.numeric(occ[(occ$Alternative == 'no_change')
                                &(occ$DayPeriod == 'Morning')
                                &(occ$weekperiod == 'weekday'),'occupancy'])

vehicle_no_change <-  vehicle_tru(no_change_2040)
thru_no_change <- agg_vehicle_thru(demand_flow_factor, no_change_2040,agg)
thru_no_change$persons <- thru_no_change$Vol * occ_no_change
...
pmt_pht_no_change <- PMT_PHT(vehicle_no_change,no_change_2040,occ_dat = occ_d)
pmt_pht_no_change['alternative'] = 'no_change'
```

```{r GP}

occ_GP <- as.numeric(occ[(occ$Alternative == 'GP')
                         &(occ$DayPeriod == 'Morning')
                         &(occ$weekperiod == 'weekday'),'occupancy'])

vehicle_GP <-  vehicle_tru(GP_2040)
thru_GP <-  agg_vehicle_thru(demand_flow_factor, GP_2040, agg)
thru_GP$persons <- thru_GP$Vol * occ_GP
...
pmt_pht_GP <- PMT_PHT(vehicle_GP,GP_2040,occ_dat = occ_d)
pmt_pht_GP['alternative'] = 'GP'
```

```{r mnpass ff}

occ_mnpass_ff <- as.numeric(occ[(occ$Alternative == 'MnPass_ff')
                                &(occ$DayPeriod == 'Morning')
                                &(occ$weekperiod == 'weekday'),'occupancy'])
...
thru_MNPass_ff$persons <- thru_MNPass_ff$Vol * occ_mnpass_ff
...
pmt_pht_mnpass_ff <- select(MNPass_2040_FF, -Demand_Hour_2040_all_lanes) %>%
  mutate(Demand_Hour_2040_all_lanes = MNPass_demand_hour_final) %>%
  PMT_PHT(vehicle_MNPass_ff,alternative = .,occ_dat= occ_d)
...
occ_mnpass_gp = as.numeric(occ[(occ$Alternative == 'MnPass_GP')
                               &(occ$DayPeriod == 'Morning')
                               &(occ$weekperiod == 'weekday'),'occupancy'])
...
vehicle_MNPass_remain <- merge(vehicle_MNPass_remain, 
  ...
  mutate(Flow_Hour_2040_all_lanes_day_dp_sum = 
           Flow_Hour_2040_all_lanes_day_dp_sum - Flow_Hour_2040_mnpass_lane_day_dp_sum,
         demand_flow_factor = Demand_Hour_2040_all_lanes_day_dp_sum /Flow_Hour_2040_all_lanes_day_dp_sum)
...
thru_MNPass_remain <- select(MNPass_2040_remaining_lanes, -Demand_Hour_2040_all_lanes) %>%
  ...
  agg_vehicle_thru(alternative = .,demand_flow_factor, agg)  
thru_MNPass_remain$persons <- thru_MNPass_remain$Vol * occ_mnpass_gp
...
pmt_pht_mnpass_remain <- select(MNPass_2040_remaining_lanes, -Demand_Hour_2040_all_lanes) %>%
  mutate(Demand_Hour_2040_all_lanes = Remaining_all_lanes_demand_hour) %>%
  PMT_PHT(vehicle_MNPass_remain,alternative = .,occ_dat=occ_d)
...
flow_mnpass_GP <- vehicle_tru_mnPass_remain(MNPass_2040_remaining_lanes,grpby = 'Hour') %>%
  ...
  select(Segment,Hour,doy,Flow_Hour_2040_all_lanes_day_dp_sum,Lanes) %>%
  mutate(mnpass_GP_flow = (Flow_Hour_2040_all_lanes_day_dp_sum/Lanes)/4)
...
```{r}
dir_len_existing <- Rel_p %>%
  filter(Segment != 1) %>%
  mutate(direction = ifelse(Segment < 6,'EB','WB')) %>%
  group_by(Segment, direction) %>%
  summarise(f = mean(FF)) %>%
  group_by(direction) %>%
  summarise(FF_direction = mean(f)) %>%
  mutate(direction_length = ifelse(direction == 'EB',8.7,9.5))

Rel_p_dir <- Rel_p %>%
  filter(Segment != 1) %>%
  mutate(direction = ifelse(Segment < 6,'EB','WB')) %>%
  merge(dir_len_existing) %>%
  mutate(ff_tt_hour = Length/FF,
         tt_hour = TTI * ff_tt_hour,
         tt_hour_pred = pred_final * ff_tt_hour) %>%
  group_by(DateTime, direction) %>%
  summarise(VMT_direction_15min = sum(VMT_total_SRF),
            TT_hour_direction_15min = sum(tt_hour),
            TT_hour_direction_15min_pred = sum(tt_hour_pred),
            direction_length  = max(direction_length),
            FF_direction = max(FF_direction)) %>%
  mutate(FF_TT_hour_direction = direction_length / FF_direction,
         TTI = TT_hour_direction_15min / FF_TT_hour_direction,
         pred_final = TT_hour_direction_15min_pred / FF_TT_hour_direction) 
```
...
vehicle_no_change <-  vehicle_tru(no_change_2040, dp=allday)
agg_nochange <- agg_vehicle_thru((vehicle_no_change),(no_change_2040), dp = allday, wkday = allweek )


vehicle_GP <- vehicle_tru(GP_2040,dp=allday)
agg_GP <- agg_vehicle_thru(vehicle_GP, GP_2040, dp=allday,wkday=allweek)


vehicle_MNPass_ff <- vehicle_tru_mnPass_ff(MNPass_2040_FF, dp=allday)
agg_mnpassff <- agg_vehicle_thru(vehicle_MNPass_ff, MNPass_2040_FF, dp=allday, wkday = allweek)


vehicle_MNPass_remain <- vehicle_tru_mnPass_remain(MNPass_2040_remaining_lanes,dp=allday)
agg_mnpass_remain <- agg_vehicle_thru(vehicle_MNPass_remain,MNPass_2040_remaining_lanes,dp=allday,wkday = allweek)
...
TT_dir_heat(Rel_p_dir,segment, model = FALSE, 'Existing')
ggsave(paste(segment,'Existing.png'))
TT_dir_heat(Rel_p_dir,segment, model = TRUE, 'Modeled Existing')
ggsave(paste(segment,'Modeled Existing.png'))
TT_dir_heat(agg_nochange, segment, model = FALSE,'No Build 2040')
ggsave(paste(segment,'No Build 2040.png'))
TT_dir_heat(agg_GP,segment, model = FALSE, 'GP 2040')
ggsave(paste(segment,'GP 2040.png'))
TT_dir_heat(agg_mnpassff,segment, model = FALSE, 'MNPass Lane 2040')
ggsave(paste(segment,'MNPass Lane 2040.png'))
TT_dir_heat(agg_mnpass_remain,segment, model = FALSE, 'MNPass 2040 GP lanes')
ggsave(paste(segment,'MNPass 2040 GP lane.png'))
```
...
names(final_thru)[6] <- 'persons'
final_transit <- read.csv('Transit_riders.csv')

final_thru <-  bind_rows(final_thru, final_transit)
final_thru$TTI_Cat <- factor(final_thru$TTI_Cat, 
                                   levels = c('>4.0 TTI',
                                               '3.5 TTI - 4.0 TTI',
                                               '3.0 TTI - 3.5 TTI',
                                               '2.5 TTI - 3.0 TTI',
...
ggplot(final_thru, aes(x=Alternative, y=persons, fill=TTI_Cat)) +
...
person_trips <- final_thru %>%
  ...
  summarise(person_trips = sum(persons))
...
```{r}
# write.csv(PTI, 'Final tables/PTI_5_10_2_7_7_23.csv')
# write.csv(person_trips, 'Final tables/person_trips_5_10_2_7_7_23.csv')
# write.csv(ff_person_trips, 'Final tables/ff_person_trips_5_10_2_7_7_23.csv')
# write.csv(final_delay,'Final tables/final_delay_5_10_2_7_7_23.csv')

# write.csv(PTI, 'Final tables/PTI_6_9_330_630_7_22.csv')
# write.csv(person_trips, 'Final tables/person_trips_6_9_330_630_7_22.csv')
# write.csv(ff_person_trips, 'Final tables/ff_person_trips_6_9_330_630_7_22.csv')
# write.csv(final_delay,'Final tables/final_delay_6_9_330_630_7_22.csv')

# write.csv(PTI, 'Final tables/PTI_730_830_1715_1815_7_22.csv')
# write.csv(person_trips, 'Final tables/person_trips_730_830_1715_1815_7_22.csv')
# write.csv(ff_person_trips, 'Final tables/ff_person_trips_730_830_1715_1815_7_22.csv')
# write.csv(final_delay,'Final tables/final_delay_730_830_1715_1815_7_22.csv')

# write.csv(pmt_pht_494, 'Final tables/pmt_pht_494_2040_allday_7_23_1.84.csv')

...
sample_data <- Rel_p %>%
  filter(Segment == 1, Delay_type == 'None', Quantile_l == q, !is.na(DC)) %>%
...
formulaExp <- lm(TTI ~ splines::bs(DC, knots = c(0.5,0.95,1.12)), data = sample_data)

sample_data$fitted <- ifelse(sample_data$DC < 0.5, 1, formulaExp$fitted.values)

x_list <- as.character(attr(formulaExp$terms, "predvars")[[3]]$knots)
x_list <- as.numeric(x_list[2:length(x_list)])

ggplot(sample_data) +
  geom_point(aes(x = DC, y = TTI), alpha = 0.25) +
  geom_line(aes(x = DC, y = fitted, color = 'red')) +
  geom_abline(slope = 0, intercept = 1) +
  geom_vline(xintercept = x_list)
